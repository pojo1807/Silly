...
parser.add_argument(
    "--sync",
    action="store_true",
    help="Sync all commands to the guild.",
)
...
class Silly(commands.Bot):
    ...
    def __init__(self) -> None:
        ...
        self.Console.set_window_title(
            f"Silly - Running on {platform.system()} {platform.release()}"
        )
        self.Console.print(
            rf"""
...
Discord.py v{discord.__version__}[/]""",
        ...
        super().__init__(
            intents=discord.Intents.all(),  # i dont know but the "all" works instead of "default"
            help_command=None,
    ...
    async def setup_hook(self):
        ...
        if args.sync:
            self.Logger.info(f"Syncing commands...")
            try:
                ...
                for cmd in sync_commands:
                    ...
                    self.slash_commands[cmd.name] = f"</{cmd.name}:{cmd.id}>"
            ...
            for name, id in INI.items("Slash_Commands"):
                self.slash_commands[name] = f"</{name}:{id}>"

...
-# Use `{PREFIX}help [category]` or `/help [category]` to see **all commands in a category**.""",
        ...
        class HelpView(View):
            def __init__(self, help_cog):
                ...
                self.add_item(
                    Button(
                        label="Vote for Silly",
                        url="https://top.gg/bot/1361293644320997598/vote",
                        style=discord.ButtonStyle.link,
                        emoji=self.help_cog.Emojis.CatEatingChips,
                        disabled=True,
                    )  # because i didnt add it on top.gg
                )

                self.add_item(
                    Button(
                        label="Star Repository",
                        url="https://github.com/pojo1807/Silly",
                        style=discord.ButtonStyle.link,
                        emoji=self.help_cog.Emojis.CatSmile,
                    )
                )
        ...
        for cog, commands_list in mapping.items():
            filtered = await self.filter_commands(
                commands_list, sort=True
            )  # Filter commands and sort that shit
            if filtered:
                Category = (
                    f"{cog.Emoji}   {cog.qualified_name}"
                    if cog
                    else f"{self.Emojis.UIA_Spinning}   Other (No Category)"
                )
                embed.description += f"\n# {Category}\n"
                for cmdidx, cmd in enumerate(filtered):
                    ...
                    (RequiredParams, OptionalParams) = (
                        [
                            f"[{param.name}]"
                            for param in cmd.clean_params.values()  # Get clean params (filtered the self and ctx)
                            if param.default
                            is param.empty  # Check if param has a default value, if not, it's required
                            and param.kind
                            in (
                                param.POSITIONAL_OR_KEYWORD,
                                param.KEYWORD_ONLY,
                            )  # Check if param is positional or keyword only
                        ],
                        [
                            f"<{param.name}>"
                            for param in cmd.clean_params.values()  # Get clean params (filtered the self and ctx)
                            if param.default
                            is not param.empty  # Check if param has a default value, if yes, it's mot required
                            and param.kind
                            in (
                                param.POSITIONAL_OR_KEYWORD,
                                param.KEYWORD_ONLY,
                    ...
                    embed.description += f"""{cmdidx+1}. {CommandName}
> {CommandDescription}
"""
...
""",
        ...
        for idx, cmd in enumerate(filtered):
            ...
            embed.description += f"""{idx+1}. {CommandName}
> {CommandDescription}
"""
...
""",
        ...
        embed.set_thumbnail(
            url="https://cdn.discordapp.com/attachments/1218440992651218959/1255879183931019294/image_2024-06-27_213452161.gif?ex=6801ac62&is=68005ae2&hm=e4bd704f6e03141cb1ff88f306e285654f41de4c45f666f0d79969317e17ce41&"
        )
    ...
    async def command_callback(self, ctx, *, command=None, ephemeral: bool = False):
        ...
        return await self.send_error_message(
            commands.CommandNotFound(
                f"**Uh oh, the command named `{command}` was not found!**"
            )
        )
...
class Utilities(commands.Cog, name="Meow Utilities"):
    ...
    @BetterCommand(
        ...
        help=f"""ฅ(=^･ω･^=)ฅ Meowww... You need help with commands?

(=｀ェ´=) Hmm... How do I explain this...
Oh! I know! Let me show you how to use these commands with Usage!

*purrrr...* Don't worry if you're confused, I'm here to help! (=^･ｪ･^=)∫""",
        usage=f"""**Just easily** use `{PREFIX}help` or `/help` to see all **categories and commands**.""",
    ...
    async def slash_help(self, ctx, *, option: str = None, ephemeral: bool = False):
        ...
        await self.help_command.command_callback(
            ctx, command=option, ephemeral=ephemeral
        )
    ...
    async def command_autocomplete(
        ...
        self.categories_and_commands = (
            self.help_command.get_categories_and_commands_from_bot(self.bot)
        )
        ...
        for category, command in self.categories_and_commands.items():
            if current.lower() in category.lower():
                choices.append(
                    app_commands.Choice(
                        name=category, value=category.strip("[]").lower()
                    )
                )
            for cmd in command:
                if current.lower() in cmd.name.lower():
                    choices.append(
                        app_commands.Choice(name=f"{PREFIX}{cmd.name}", value=cmd.name)
                    )
    ...
    @BetterCommand(
        name="debug",
        description="Let me show you how I'm doing! I'll tell you all about my health and status.",
        brief="Check Silly's vital signs and status",
        help="""Meowww... You want to check my status?
